...
from .slot_class import IntegerRangeField
...
class Wafer(models.Model):
    ...
    #dep_foup=models.CharField(max_length=5,blank=True,null=True,default='')
...
class Foup_slot(models.Model):
	...
	# Slots=(
		...
		# ('25', 'Slot 25'),
	# )
	slot=IntegerRangeField(min_value=1,max_value=25)
	def __str__(self):
		return (self.foup +" Slot "+str(self.slot))
	...
	def reclaim(self):
		...
		if len(corresponding_wafer)==0:
			...
			if len(corresponding_wafer)>1:
				...
				print ("%s wafer is successfully reclaimed. Now the slot is vacant." %(self.foup + ' S'+ str(self.slot)))
				return
	...
	def load_all_slots_in_foup(cls,foup_name,wafer_type):
		if not cls.foup_vacant(foup_name):
			...
			for slot in cls.all_slots(foup_name):
	...
	@classmethod
	def format_foup_name(cls,foup_name):
		#format the foup name to 'F' + 3 or 4 digits
		length=len(foup_name)
		if length>5:
			print "please limit foup name to 5 characters."
			return '0'	#given foup name is wrong, so return '0' to stand for this case
		if not foup_name[0] in 'fF':
			print "please use foup name starting with \'F\'."
			return '0'
		for i in range(1,length):	#to guarantee that the foup name is 'F' followed by 3-4 digits.
			if foup_name[i]>'9' or foup_name[i]<'0':
				print "please use 3-4 digits following 'F' for foup name."
				return '0'
		if foup_name[0]=='f':
			foup_name[0]='F'	#keep all foup name starting with capital 'F'
		return foup_name	# now the foup name is in the wanted format


	@classmethod
	def initiate_foup(cls,foup_name):
		#create the 25 slots of this foup in database
		foup=cls.format_foup_name(foup_name)
		if foup!='0':	#provided that the given foup_name is not a unformatable one.
			for i in range (1,26):
				slot=Foup_slot(foup=foup,slot=i)
				slot.save()
			return
		print "Foup name is not correct."
		return
